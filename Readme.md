...
O programa recebe do usuário as seguintes entradas: CPF, NOME COMPLETO, DATA DE NASCIMENTO
e PROFISSÃO, durante esse período o usuário pode digitar -1 para sair. Após
receber essas quatro primeiras entradas será feita uma validação, caso alguma entrada
...
entrada exigida. Então será pedido que seja respondida com (s/n) a pergunta: "Você faz parte
...
Depois do usuário inserir a senha e confirmá-la será verificado se são iguais e atendem ao
padrão, caso não sejam iguais será solicitado ao usuário que repita o processo, caso não
atendam ao padrão o programa iniciará do zero. Se nada disso acontecer será exibida a pergunta:
...
O arquivo usuarios_cadastrados.py é responsável por armazenar e exibir dados dos usuários.
